...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:param name="SRC_LANG">SRC_LANG</xsl:param>
    <xsl:param name="TGT_LANG">SRC_LANG</xsl:param>
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//entry">
            <xsl:variable name="src">
                <xsl:variable name="tmp" select="src/text()[1]"/>
                <xsl:value-of select="replace(normalize-space(translate($tmp,'&quot;;',' ,')),' *\.$','')"/>
            </xsl:variable>
            <xsl:variable name="tgt">
                <xsl:variable name="tmp" select="tgt/text()[1]"/>
                <xsl:value-of select="replace(normalize-space(translate($tmp,'&quot;;',' ,')),' *\.$','')"/>
            </xsl:variable>
            <xsl:if test="string-length($src)>0 and string-length($tgt)>0">                
                <xsl:text>&#10;# </xsl:text>
                ...
                <xsl:variable name="writtenRep" select="
                    normalize-space(
                        replace(
                        replace(
                        replace(
                ...
                <xsl:variable name="conceptLabel" select="
                    normalize-space(
                        replace(
                        replace(
                        replace(
                ...
                <xsl:text> .&#10;</xsl:text>
                <xsl:value-of select="$concept"/>
                <xsl:text> a ontolex:LexicalConcept</xsl:text>
